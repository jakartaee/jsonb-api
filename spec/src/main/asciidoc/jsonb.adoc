...
Optional creator parameter is parameter which is not required to be present in the JSON document and if it is missing, not exception will be thrown. Value `null` is provided instead of any specific value. If the parameter type is `Optional`, empty `Optional` is used instead.

It is possible to mark all `JsonbCreator` parameters as optional globally, by calling the`JsonbConfig::withOptionalCreatorParameters` method with either `true` to enable or `false` to disable this feature. Default value is `false`. When set to `true`, every `JsonbCreator` parameter is treated as optional, unless the `JsonbCreator` overrides this setting.

Implementation must allow setting all parameters to be optional or required only for certain creator. This is achieved by setting `parameters` property of `JsonbCreator` annotation to `ParameterState.NILLABLE` or `ParameterState.REQUIRED`. `ParameterState.DEFAULT` option takes the value from the JSONB config.

Implementation must allow making only certain parameters as optional, without setting the global property or creator specific property.

Parameter can be declared an optional:

* Object types have to be annotated with annotation `JsonbNillable`
