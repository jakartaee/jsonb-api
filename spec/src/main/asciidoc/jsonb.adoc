...
In case a field required for a parameter mapping doesn’t exist in JSON document, `JsonbException` MUST be thrown unless it is marked as an optional. See <<optional-creator-params, Optional JsonbCreator parameters>>

[[optional-creator-params]]
==== Optional JsonbCreator parameters
Optional creator parameter is parameter which is not required to be present in the JSON document and if it is missing, no exception will be thrown.

It is possible to mark all `JsonbCreator` parameters as optional globally, by calling the`JsonbConfig::withOptionalCreatorParameters` method with either `true` to enable or `false` to disable this feature. Default value is `false`. When set to `true`, every `JsonbCreator` parameter is treated as optional.

Implementation must allow setting all parameters to be optional or required only for certain creator. This is achieved by adding `JsonbRequired` annotation to the creator method/constructor. If value of this annotation is set to false, all parameters of the creator will be treated as optional.

Implementation must allow making only certain parameters as optional/required, without setting the global property or creator specific property.

Parameter can be declared an optional:

* Object types have to be annotated with annotation `JsonbRequired`
* Parameter is `Optional` type

If property has explicit value `null` set in the JSON document and parameter is primitive type, implementation must throw an exception.

==== Optional parameter values
Whenever a property is missing from JSON document and is marked as optional, proper default value should be used. If the parameter is type of any Object except `Optional` and its variations, null value is used. In case of `Optional` and its variations, corresponding empty object must be used.

Primitive types cannot accept null values, and therefore it is required to provide corresponding value, as listed in the following table.
