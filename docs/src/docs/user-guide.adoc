:component-name: Json Binding {revnumber} Users Guide
:toc: left
:toclevels: 3
...
.Maven Dependencies
----
include::../include/pom.xml[tag=dep]
----
...
- On record component accessor: Property is ignored during serialization only.
- On record virtual attribute: Property is ignored during serialization only.
