...
==== Record Virtual Attributes

A _record virtual attribute_ is a value-returning, no-argument method of a record type where the method is declared `public`, but not `static`, and whose name differs from the names of components of the record. Additionally, methods from `java.lang.Object` and overrides thereof, such as `String toString()`, are excluded from being _record virtual attributes_.

Implementations MUST include all public virtual attributes in the serialized JSON output of a record unless suppressed by `@JsonbTransient`.

The JSON property name for a virtual attribute is the method name unchanged (identity transformation, consistent with <<names-identifiers,Names and Identifiers>>).

The JSON property value is the return value of the method, serialized according to the type's default or customized mapping.

Virtual attributes are read-only.
A JSON name/value pair whose key matches the name of a virtual attribute MUST be silently ignored during deserialization, consistent with the <<must-ignore-policy,Must-Ignore Policy>>.
No exception is thrown when such a pair is present in the input document.

[source,java]
----
public record FullName(String first, String last) {
    // virtual attribute — not a record component accessor
...
Serializing `new FullName("Jason", "Bourne")` with default mapping produces:

[source,json]
----
{"first":"Jason","last":"Bourne","display":"Jason Bourne"}
----

Deserializing the prior JSON into `FullName` silently ignores the `display` property and produces an instance of `FullName [ first="Jason", last="Bourne" ]`.
...
Public getter/setter methods without a corresponding field MUST be supported when serializing and deserializing Java Bean classes.
...
and the setter method is called during the deserialization operation.

For a deserialization operation of a Java record component:

* If a custom constructor or factory method as described in <<custom-instantiation,Custom Instantiation>> is present, then a custom constructor or factory method is called to set the value of the component.
* If no custom constructor or factory method exists, then the canonical constructor as described in <<java-class-and-record,Java Class and Record>> is called to set the value of the component.

For a serialization operation of a Java record component:

* The component accessor method is called to obtain the value of the component.  The component accessor method may be compiler-generated or explicitly redeclared by the record author.
...
Implementations MUST support the binding of public and protected nested classes/records. For deserialization operations, both nested and encapsulating classes/records MUST fulfill the same instantiation requirements as specified in 3.7.1.

==== Static Nested Classes

Implementations MUST support the binding of public and protected static nested classes/records. For deserialization operations, the nested class/records MUST fulfill the same instantiation requirements as specified in 3.7.1.
...
Polymorphic type handling is supported for deserialization and serialization. Polymorphic handling is ensured by annotation `JsonbTypeInfo` and `@JsonbSubtype`. `JsonbTypeInfo` defines key name of the property to store type information in it and defines all the supported aliases using `@JsonbSubtype` annotations. `@JsonbSubtype` ensures proper and safe mapping between class/record alias and type. Implementation must validate mapped types if they are assignable from the annotated type. If not, an exception must be thrown.

Type information is obtained from `@JsonbSubtype` annotation as a type alias mapped to the type. If no matching type is found for obtained alias during deserialization, an exception must be thrown.

New property with type information is added to the serialized object. The property key name is taken from the `key` property of the annotation `JsonbTypeInfo`. This type information property key name has to be unique in the resulting JSON document. If any naming collision with class/record or any other `JsonbTypeInfo` properties occurs, an exception must be thrown. It is required for all polymorphism fields to be serialized as the first properties in the JSON and any actual object properties are serialized after.

If no `JsonbTypeInfo` is used on handled class/record or its predecessors, it is not possible to ensure proper polymorphic type handling and in such cases deserialization is not supported.
...
If a class/record property is defined with an interface and not concrete type, then the mapping for a serialized property is resolved based on its runtime type.
...
Java properties and record components as described in <<scope-and-field-access-strategy,Scope and Field access strategy>> MUST be serialized in lexicographical order into the resulting JSON document.
In case of inheritance, properties declared in the super class MUST be serialized before properties declared in a child class.
...
There are several levels of information JSON Binding implementations may obtain about the type of field/class/record/interface:
...
If only raw type of given field/class/record/interface is known, then the type MUST be treated like raw type. For example, if the only available information is that given field/class/record/interface is of type `java.util.ArrayList`, then the type MUST be treated as `java.util.ArrayList<Object>`.

JSON Binding implementations MUST use the most specific type derived from the information available.

Let's consider situation when there is only a static type information of a given field/class/record/interface known, and there is no runtime type information available.
...
If `B~1~` is a class, record or interface, the most specific parameter type MUST be the class, record or interface.
...
If `S~1~,…,S~m~` are `java.lang.Object`, then the bounded parameter `T~i~` type MUST be `java.lang.Object`.

If there is exactly one `S~k~`, where `1<=k<=m` is different than `java.lang.Object`, then the most specific parameter type for this bounded parameter `T~i~` type MUST be `S~k~`.
...
This section defines several ways how to customize the default behavior. The default behavior can be customized annotating a given field, record component, JavaBean property, type or package, or by providing an implementation of particular strategy, e.g. `PropertyOrderStrategy`. JSON Binding provider MUST support these customization options.

=== Customizing Property Names

There are two standard ways how to customize serialization of field or record component (or JavaBean property) to JSON document. The same applies to deserialization. The first way is to annotate field or record component (or JavaBean property) with `jakarta.json.bind.annotation.JsonbProperty` annotation. The second option is to set `jakarta.json.bind.config.PropertyNamingStrategy`.

==== jakarta.json.bind.annotation.JsonbTransient

JSON Binding implementations MUST NOT process fields, record components, JavaBean properties or types annotated with `jakarta.json.bind.annotation.JsonbTransient`.
...
** Exception must be thrown when this field, its getter, or its setter is annotated with other JSON Binding annotations.
* Getter is annotated with `@JsonbTransient`
** An exception is thrown if the field or this getter are annotated with other JSON Binding annotations. An exception is not thrown if JSON Binding annotations are present on the setter.
* Setter is annotated with `@JsonbTransient`
** An exception is thrown if the field or this setter are annotated with other JSON Binding annotations. An exception is not thrown if JSON Binding annotations are present on the getter.
* Record component is annotated with `@JsonbTransient`
** An exception must be thrown when this component or its accessor is annotated with other JSON Binding annotations.
* Record component accessor is annotated with `@JsonbTransient`
** An exception is thrown if the corresponding component or this accessor is annotated with other JSON Binding annotations.
* Record virtual attribute method is annotated with `@JsonbTransient`
** The method is excluded from serialization output.
** An exception must be thrown if the method is also annotated with any other JSON Binding annotation.

==== jakarta.json.bind.annotation.JsonbProperty

According to default mapping 3.15, property names are serialized unchanged to a JSON document (identity transformation). To provide a custom name for a given field or record component (or JavaBean property), `jakarta.json.bind.annotation.JsonbProperty` may be used. The `JsonbProperty` annotation may be specified on a field, record component, record component accessor method, virtual attribute method, getter or setter method, or a `@JsonbCreator` annotated constructor or static factory method parameter.

If specified on field or record component, custom name is used both for serialization and deserialization.
...
If `jakarta.json.bind.annotation.JsonbProperty` is specified on a virtual attribute method or component accessor method, then the customization defined by the `JsonbProperty` annotation applies only for serialization.

If `jakarta.json.bind.annotation.JsonbProperty` is specified on a parameter of a `@JsonbCreator` annotated constructor or static factory method, the custom name is used only for deserialization.

It is possible to specify different values for getter and setter method for `jakarta.json.bind.annotation.JsonbProperty` annotation. In such case the different custom name will be used for serialization and deserialization independently.

=== jakarta.json.bind.annotation.JsonbPropertyOrder

To customize the order of serialized properties only for one specific type, JSON Binding provides the `jakarta.json.bind.annotation.JsonbPropertyOrder` annotation. The order specified by the `JsonbPropertyOrder` annotation overrides the order specified by `PropertyOrderStrategy` for that type.

Properties listed in `@JsonbPropertyOrder` are serialized first in the declared order. Any remaining properties not listed are serialized afterward in an unspecified order. Names provided in `@JsonbPropertyOrder` MUST correspond to the original property or record component names as defined in the type, before any renaming customization (such as `@JsonbProperty` or a `PropertyNamingStrategy`) is applied.

==== jakarta.json.bind.config.PropertyNamingStrategy

To customize name translation of properties, JSON Binding provides the `jakarta.json.bind.config.PropertyNamingStrategy` interface.

A naming strategy transforms the _identity property name_ into the JSON property name used during serialization and deserialization. The identity property name is determined by one of the following program elements: 

- a field name
- a JavaBean property name
- a record component name
- a record virtual attribute name.

A `jakarta.json.bind.annotation.JsonbProperty` annotation on an individual field, record component, accessor method, virtual attribute, or `@JsonbCreator` parameter takes precedence over the naming strategy for that property. The strategy is applied only when no such annotation is present.

Interface `jakarta.json.bind.config.PropertyNamingStrategy` provides the following predefined naming strategies:
...
To set a property naming strategy use the `jakarta.json.bind.JsonbConfig::withPropertyNamingStrategy` method.
...
Class `jakarta.json.bind.config.PropertyOrderStrategy` provides the following predefined property order strategies:
...
Use the `jakarta.json.bind.JsonbConfig::withPropertyOrderStrategy` method to configure a property order strategy.

=== Customizing Null Handling

There are two main ways how to change default null handling. The first option is to annotate type, package, field, record component or JavaBean property with `jakarta.json.bind.annotation.JsonbNillable` annotation. The second option is to set config-wide configuration via `JsonbConfig::withNullValues` method. There is also a third option to annotate a field, record component or JavaBean property with `jakarta.json.bind.annotation.JsonbProperty` and to set nillable parameter to true. However, this option is currently deprecated, and it is recommended to use `jakarta.json.bind.annotation.JsonbNillable` instead.

If annotations (`JsonbNillable` or `JsonbProperty`) on different level apply to the same field (or JavaBean property), record component or if there is config wide configuration and some annotation (`JsonbNillable` or `JsonbProperty`) which apply to the same field or record component (or JavaBean property), the annotation with the smallest scope applies. For example, if there is type level `JsonbNillable` annotation applied to some class with field which is annotated with `JsonbProperty` annotation with nillable = false, then `JsonbProperty` annotation overrides `JsonbNillable` annotation.
...
To customize the result of serializing field or record component (or JavaBean property) with null value, JSON Binding provides `jakarta.json.bind.annotation.JsonbNillable` annotation.

When a given object (type, package, field, record component, virtual attribute, or JavaBean property) is annotated with `jakarta.json.bind.annotation.JsonbNillable` annotation, then a value of `null` will be explicitly serialized to `null` in the JSON document, instead of being skipped as described in <<null-java-field, Null Java Field>>.

The same behavior as `JsonbNillable`, but only at field, record component, parameter and method (JavaBean property) level is provided by `jakarta.json.bind.annotation.JsonbProperty` annotation with its `nillable` parameter. This option is currently deprecated, and it is recommended to use `jakarta.json.bind.annotation.JsonbNillable` instead.
...
. Annotating a class field/record component with `JsonbTypeAdapter` annotation.

`JsonbAdapter` registered via `JsonbConfig::withAdapters` is visible to all serialize/deserialize operations performed with given `JsonbConfig`. `JsonbAdapter` registered with annotation is visible to serialize/deserialize operation used only for annotated field/record component.
...
. Annotating a type with `@JsonbTypeSerializer`/`@JsonbTypeDeserializer` annotation.
...
If `jakarta.json.bind.annotation.JsonbDateFormat` is specified on a getter, virtual attribute, or component accessor method, it is used only for serialization. If `jakarta.json.bind.annotation.JsonbDateFormat` is specified on a setter method, it is used only for deserialization.
...
If `jakarta.json.bind.annotation.JsonbNumberFormat` is specified on a getter, virtual attribute, or component accessor method, it is used only for serialization. If `jakarta.json.bind.annotation.JsonbNumberFormat` is specified on a setter method, it is used only for deserialization.
