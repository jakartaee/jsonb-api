...
* Property - new property with polymorphic information is added to the serialized object. The property key is taken from the `keyName` of the annotation `@JsonbPolymorphicType`
...
* Wrapping array - serialized object is wrapped by the array. This array contains only two values. The first MUST be polymorphic information of the type and the second value is the serialized object
