...
Polymorphic type handling is supported for deserialization and serialization. Polymorphic handling is ensured by annotation '@JsonbPolymorphicType' and '@JsonbSubtype'. '@JsonbPolymorphicType' defines overall strategy how polymorphic information will be serialized/deserialized. '@JsonbSubtype' ensures proper and safe mapping between class alias and type.

Polymorphic information is obtained from '@JsonbSubtype' annotation as a type alias mapped to the type or if classNames property of the '@JsonbPolymorphicType' is enabled, it corresponds to the class name, if no suitable alias is found. Implementation MUST ensure to support 'allowedPackages' option of the '@JsonbPolymorphicType' which is only used when 'classNames' option is enabled, it is ignored otherwise. It is not allowed to create instances of the classes outside the whitelisted packages. In such cases an exception MUST be thrown.

Available serialization formats:

* Property - new property with polymorphic information is added to the serialized object. Serialized polymorphic information has to be the first property in the object regardless of the specified property order strategy or property order
* Wrapping object - serialized object is wrapped with another object with a single property. This key corresponds to the polymorphic information of the type and the value is the serialized object
* Wrapping array - serialized object is wrapped by the array. This array contains only two values. The first MUST be polymorphic information of the type and the second value is the object itself
