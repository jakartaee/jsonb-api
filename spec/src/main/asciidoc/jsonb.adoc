...
There are two ways to configure default null handling. The first option is to annotate type, package, field, record component or JavaBean property with the `jakarta.json.bind.annotation.JsonbNillable` annotation. The second option is to set the global configuration via `JsonbConfig.withNullValues` method.

If @JsonbNillable applies at different levels to the same field, JavaBean property, or record component, then the setting with the smallest scope takes precedence. Where the largest scope is given to global configuration via `JsonbConfig::withNullValues`. For example, a package-level @JsonbNillable annotation is overridden by a type-level @JsonbNillable annotation.
