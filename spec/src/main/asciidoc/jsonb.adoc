...
Serializing `new FullName("Jason", "Borne")` with default mapping produces:

[source,json]
----
{"first":"Jason","last":"Borne","display":"Jason Borne"}
----

Deserializing the prior JSON into `FullName` silently ignores the `display` property and produces an instance of `FullName [ first="Jason", last="Borne" ]`.
