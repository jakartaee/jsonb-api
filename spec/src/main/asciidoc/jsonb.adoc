...
A _record virtual attribute_ is a public, non-static, no-argument instance method declared on a record type that is not the compiler-generated component accessor for any record component of that record and does not return `void`.
...
Public getter/setter methods without a corresponding field MUST be supported when serializing and deserialzing Java Bean classes.
...
* If a custom constructor or factory method as described in <<custom-instantiation,Custom Instantiation>>, then custom constructor or factory method is called to set the value of the component.
...
In case of inheritance, properties declared in super class MUST be serialized before properties declared in a child class.
...
According to default mapping 3.15, property names are serialized unchanged to JSON document (identity transformation). To provide custom name for given field or record component (or JavaBean property), `jakarta.json.bind.annotation.JsonbProperty` may be used. `JsonbProperty` annotation may be specified on field, record component, virtual attribute method, component accessor method, getter or setter method.
...
If `jakarta.json.bind.annotation.JsonbProperty` is specified on a virtual attribute method or component accessor method, then it is used only for serialization.
