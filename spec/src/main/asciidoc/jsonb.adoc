...
If no `JsonbTypeInfo` is used on handled class or its predecessors, it is not possible to ensure proper type inheritance handling and in such cases deserialization is not supported.

If there are multiple different type information customizations that need to be merged, an exception must be thrown. Multiple inheritance of this customization is not supported.
...
----
The order of the type information properties must be the same in which they appear in the type chain. Resulting JSON when serializing MyCar class would look like this:
[source,json]
----
{
    "@vehicle" : "car",
