...
In case a field required for a parameter mapping doesn't exist in JSON document, `JsonbException` MUST be thrown unless it is marked as an optional. See <<optional-creator-params, Optional JsonbCreator parameters>>. All the parameters are treated as optional by default.
...
Implementation must allow setting all parameters to be required of a certain creator. This is achieved by adding `JsonbRequired` annotation to the creator method/constructor. If value of this annotation is set to false, all parameters of the creator will be treated as optional.

Implementation must allow making only certain parameters as optional/required. Parameter can be declared as required if annotated with `JsonbRequired`.

Parameter can be declared as optional by using:

* Parameter annotated with `JsonbRequired` and having value set to `false`
