...
include::../generated/packages.adoc[]
...
installation instructions. See <<Installation>>, for instructions on 
installing the {TechnologyShortName} TCK.
2.  Set up the {TechnologyShortName} TCK software. +
See <<Setup and Configuration>>, for
...
the test suite.  See <<Executing Tests>>.
