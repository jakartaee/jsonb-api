...
Polymorphic information is obtained from `@JsonbSubtype` annotation as a type alias mapped to the type. If no matching alias is found for the serialized instance, an exception must be thrown. If no matching class is found for obtained alias from the JSON, an exception must be thrown.
