...
There are two main ways how to change default null handling. The first option is to annotate type, package, field, record component or JavaBean property with `jakarta.json.bind.annotation.JsonbNillable` annotation. The second option is to set config-wide configuration via `JsonbConfig::withNullValues` method.

If null-serialization rules (JsonbNillable) apply at different levels to the same field, JavaBean property, or record component-or between a configuration-wide setting and an annotation-the setting with the smallest scope takes precedence. For example, a type-level @JsonbNillable annotation applied to a class overrides a configuration-wide nillable setting for fields within that class.
